...
<form method="POST" action="{{ route('admin.profil.update') }}" enctype="multipart/form-data" class="space-y-6 max-w-3xl">
    ...
    <div class="bg-white rounded-2xl border border-emerald-900/10 p-6 space-y-4">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nama_desa" value="{{ old('nama_desa', $profil->nama_desa) }}" class="w-full rounded-lg border-emerald-900/20 text-sm" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Kecamatan</label>
                <input type="text" name="kecamatan" value="{{ old('kecamatan', $profil->kecamatan) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Kabupaten</label>
                <input type="text" name="kabupaten" value="{{ old('kabupaten', $profil->kabupaten) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Provinsi</label>
                <input type="text" name="provinsi" value="{{ old('provinsi', $profil->provinsi) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Luas Wilayah</label>
                <input type="text" name="luas_wilayah" value="{{ old('luas_wilayah', $profil->luas_wilayah) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Jumlah Penduduk</label>
                <input type="text" name="jumlah_penduduk" value="{{ old('jumlah_penduduk', $profil->jumlah_penduduk) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-emerald-900/10 p-6 space-y-4">
        ...
        <div>
            ...
            <textarea name="sejarah" rows="4" class="w-full rounded-lg border-emerald-900/20 text-sm">{{ old('sejarah', $profil->sejarah) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-emerald-950/80 mb-1">Visi</label>
            <textarea name="visi" rows="2" class="w-full rounded-lg border-emerald-900/20 text-sm">{{ old('visi', $profil->visi) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-emerald-950/80 mb-1">Misi</label>
            <textarea name="misi" rows="4" class="w-full rounded-lg border-emerald-900/20 text-sm">{{ old('misi', $profil->misi) }}</textarea>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-emerald-900/10 p-6 space-y-4">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                @endif
                <input type="file" name="foto_desa" accept="image/*" class="w-full text-sm">
            </div>
    ...
    <div class="bg-white rounded-2xl border border-emerald-900/10 p-6 space-y-4">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nama_kontak" value="{{ old('nama_kontak', $profil->nama_kontak) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Jabatan</label>
                <input type="text" name="jabatan_kontak" value="{{ old('jabatan_kontak', $profil->jabatan_kontak) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">No. Telepon</label>
                <input type="text" name="no_telepon" value="{{ old('no_telepon', $profil->no_telepon) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Email</label>
                <input type="email" name="email" value="{{ old('email', $profil->email) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div class="sm:col-span-2">
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Alamat Kantor</label>
                <textarea name="alamat_kantor" rows="2" class="w-full rounded-lg border-emerald-900/20 text-sm">{{ old('alamat_kantor', $profil->alamat_kantor) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Jam Pelayanan</label>
                <input type="text" name="jam_pelayanan" value="{{ old('jam_pelayanan', $profil->jam_pelayanan) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Link Google Maps (opsional)</label>
                <input type="text" name="link_maps" value="{{ old('link_maps', $profil->link_maps) }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
            </div>
        </div>
    </div>
