...
@section('content')
<form method="POST" action="{{ route('admin.profil.update') }}" enctype="multipart/form-data" class="space-y-4">
    @csrf
    @method('PUT')

    <div class="grid lg:grid-cols-2 gap-4 items-start">
        <div class="bg-white rounded-xl border border-emerald-900/10 p-4 space-y-3">
            <h2 class="font-semibold text-sm">Data Umum</h2>
        ...
        <div class="bg-white rounded-xl border border-emerald-900/10 p-4 space-y-3">
            <h2 class="font-semibold text-sm">Kontak Person</h2>
            <div class="grid sm:grid-cols-2 gap-3">
                <div>
        ...
        <div class="bg-white rounded-xl border border-emerald-900/10 p-4 space-y-3">
            <h2 class="font-semibold text-sm">Foto</h2>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Logo Desa</label>
                <p class="text-xs text-emerald-900/40 mb-1.5">Tampil di header &amp; footer website sebagai pengganti lencana singkatan "TA".</p>
                <div class="flex items-center gap-3">
                    @if ($profil->logo)
                        <img src="{{ asset('storage/'.$profil->logo) }}" class="h-12 w-12 shrink-0 object-cover rounded-full border border-emerald-900/10">
                    @endif
            ...
            <div>
                ...
                <p class="text-xs text-emerald-900/40 mb-1.5">Gambar ini tampil sebagai latar belakang penuh di halaman utama. Jika kosong, sistem memakai gambar contoh bawaan.</p>
                @if ($profil->foto_hero)
                    <img src="{{ asset('storage/'.$profil->foto_hero) }}" class="h-20 w-full object-cover mb-1.5 rounded-lg">
                @else
                    <img src="{{ asset('img/kerja-kerja.jpg') }}" class="h-20 w-full object-cover mb-1.5 rounded-lg opacity-70">
                @endif
        ...
        <div class="bg-white rounded-xl border border-emerald-900/10 p-4 space-y-3">
            <h2 class="font-semibold text-sm">Sejarah &amp; Visi Misi</h2>
            <div>
                <label class="block text-sm font-medium text-emerald-950/80 mb-1">Sejarah</label>
                <textarea name="sejarah" rows="3" class="form-input">{{ old('sejarah', $profil->sejarah) }}</textarea>
            </div>
            <div>
    ...
    <button type="submit" class="rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 text-sm font-medium transition-colors">Simpan Perubahan</button>
</form>
@endsection
