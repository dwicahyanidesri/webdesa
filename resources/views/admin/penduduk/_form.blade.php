...
<div class="grid sm:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <input type="text" name="nik" value="{{ old('nik', $item->nik ?? '') }}" maxlength="20" class="w-full rounded-lg border-emerald-900/20 text-sm">
    </div>
    <div>
        ...
        <select name="jenis_kelamin" class="w-full rounded-lg border-emerald-900/20 text-sm">
    ...
    <div>
        ...
        <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', isset($item) && $item->tanggal_lahir ? $item->tanggal_lahir->format('Y-m-d') : '') }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
    </div>
    <div>
    ...
    <div>
        ...
        <select name="status_keluarga" class="w-full rounded-lg border-emerald-900/20 text-sm">
    ...
    <div>
        ...
        <select name="status_ekonomi" class="w-full rounded-lg border-emerald-900/20 text-sm">
    ...
    <div>
        ...
        <select name="status_nikah" class="w-full rounded-lg border-emerald-900/20 text-sm">
    ...
    <div>
        ...
        <select name="pendidikan_terakhir" class="w-full rounded-lg border-emerald-900/20 text-sm">
    ...
    <div class="sm:col-span-2">
        ...
        <input type="text" name="alamat" value="{{ old('alamat', $item->alamat ?? '') }}" class="w-full rounded-lg border-emerald-900/20 text-sm">
    </div>
</div>
...
<div>
    <label class="inline-flex items-center gap-2 text-sm text-emerald-950/80">
        ...
        <input type="checkbox" name="status_sekolah" value="1" class="rounded border-emerald-900/20 text-emerald-600 focus:ring-emerald-500" @checked($sekolah)>
        Sedang bersekolah
    </label>
</div>
    ...
    <div class="flex flex-wrap gap-3">
        @foreach (\App\Models\Penduduk::BANTUAN as $jenis)
            <label class="inline-flex items-center gap-2 text-sm rounded-lg border border-emerald-900/15 px-3 py-1.5">
                <input type="checkbox" name="penerima_bantuan[]" value="{{ $jenis }}" class="rounded border-emerald-900/20 text-emerald-600 focus:ring-emerald-500" @checked(in_array($jenis, $bantuanTerpilih ?? []))>
                {{ $jenis }}
            </label>
        @endforeach
    </div>
